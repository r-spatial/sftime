...
x_sftime4 <- 
  st_sftime(
    a = 1:50,
    b = rnorm(50),
    id_object = as.factor(sample(x = 1:4, size = 50, replace = TRUE)),
    geometry = g, 
...
as.data.frame.interval = function(x, ...) data.frame(as.character(x))

sf = st_sf(a = 1:4, geom = g)
sf$time = tc
#(sft <- st_sftime(a = 1:4, g, time = sort(tc)))
# sort(tc) returns a list, which can not be sorted as it is unclassed
(sft <- st_sftime(sf, time_column_name = "time"))
```
